...
async def register_user(user_id, first_name, last_name, phone, gender, age):
    async with aiosqlite.connect(DB_PATH) as db:
        await db.execute("""
            ...
            (user_id, first_name, last_name, phone, gender, age, registered_at)
            VALUES (?, ?, ?, ?, ?, ?, ?)
...
async def add_lesson(date, time, duration, price, description):
    async with aiosqlite.connect(DB_PATH) as db:
        cursor = await db.execute("""
            INSERT INTO lessons (date, time, duration, price, description)
            VALUES (?, ?, ?, ?, ?)
...
async def book_lesson(user_id, lesson_id):
    async with aiosqlite.connect(DB_PATH) as db:
        ...
        await db.execute(
            "UPDATE lessons SET is_available = 0 WHERE id = ?",
            (lesson_id,)
        )
        await db.commit()
...
async def get_user_bookings(user_id):
    async with aiosqlite.connect(DB_PATH) as db:
        async with db.execute("""
            SELECT b.id, l.date, l.time, l.duration,
                   l.price, l.description, b.status
            FROM bookings b
...
async def update_lesson(lesson_id, date=None, time=None,
                        duration=None, price=None, description=None):
...
async def get_all_bookings():
    async with aiosqlite.connect(DB_PATH) as db:
        async with db.execute("""
            SELECT b.id, u.first_name, u.last_name, u.phone,
                   l.date, l.time, l.price, b.status
            FROM bookings b
